...
void SubArray::Initialize(long long _numRow, long long _numColumn, bool _multipleRowPerSet, bool _split,
	...
	if (cell->memCellType == MRAM || cell->memCellType == PCRAM || cell->memCellType == memristor||cell->memCellType==DWM) {
		if (cell->accessType == CMOS_access){
			if (tech->currentOnNmos[inputParameter->temperature - 300]
				...
				//cout << "currentOnNmos :  " << tech->currentOnNmos[inputParameter->temperature - 300] << endl;
				//cout << "currentOffNmos :  " << tech->currentOffNmos[inputParameter->temperature - 300] << endl;
				//cout << "if statement : " << tech->currentOnNmos[inputParameter->temperature - 300]
				//					/ tech->currentOffNmos[inputParameter->temperature - 300] << endl;
				/* bitline too long */
				invalid = true;
				initialized = true;
				return;
			}
			maxBitlineCurrent = MAX(cell->resetCurrent, cell->setCurrent) + cell->leakageCurrentAccessDevice * (numRow - 1); // worse case scenerio of programming the cell
	...
	if (minBitlineMuxWidth > inputParameter->maxNmosSize * tech->featureSize) {
		cout << "[Subarray] Error: Bitline mux size is too large!" << endl;
		cout << "maxBitlineCurrent: " << maxBitlineCurrent << endl;
		cout << "minBitlineMuxWidth: " << minBitlineMuxWidth << endl;
		cout << "maxNmosSize * featureSize: " << inputParameter->maxNmosSize * tech->featureSize << endl;
		cout << "currentOnNmos: " << tech->currentOnNmos[inputParameter->temperature - 300] << endl;
		invalid = true;
		return;
	}
